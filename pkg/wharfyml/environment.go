...
func parseStageEnvironments2(node ast.Node) (envs []string, errSlice errorSlice) {
	if node.Type() != ast.SequenceType {
		return nil, errorSlice{wrapParseErrNode(ErrStageEnvsNotArray, node)}
	}
	...
	for _, envNode := range seq.Values {
		...
		if !ok {
			errSlice.add(wrapParseErrNode(ErrStageEnvNotString, envNode))
			continue
		}
		if envStrNode.Value == "" {
			errSlice.add(wrapParseErrNode(ErrStageEnvEmpty, envNode))
			continue
		}
