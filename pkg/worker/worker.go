...
type StageRunnerFactory interface {
	NewStageRunner(ctx context.Context, stage wharfyml.Stage, startStepID int) (StageRunner, error)
}
...
type StepRunnerFactory interface {
	NewStepRunner(ctx context.Context, step wharfyml.Step, stepID int) (StepRunner, error)
}
